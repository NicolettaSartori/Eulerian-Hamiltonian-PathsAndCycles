...
def nx_to_graphviz(nx_graph, labeled_edges=False, labeled_nodes=True, start_node=None, center_lables=False):
    ...
    for edge in nx_graph.edges():
        if labeled_edges:
            if center_lables:
                g.edge(str(edge[0]), str(edge[1]), label=chr(ord("a") + idx), labeldistance="1.5")
            else:
...
def nx_to_graphviz_with_marked_path(nx_graph, path=[], labeled_edges=False, labeled_nodes=True, start_node=None  ):
    ...
    edge_path = nx.MultiGraph()
    for i in range(0, len(path)-1):
        edge_path.add_edge(path[i], path[i+1])
    ...
    for edge in nx_graph.edges(keys=True):
        if edge_path.has_edge(edge[0], edge[1], edge[2]):
            if labeled_edges:
