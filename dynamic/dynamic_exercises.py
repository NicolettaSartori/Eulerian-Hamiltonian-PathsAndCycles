...
from create_xml import format_draggables, format_mc_stage, format_mc_stage_dynamic, format_mc_stage_dynamic_tf
...
def export_exercise(generate_fn, title, article, type, filename):
    ...
    # Use original edge-based formatting helper (generate_fn may produce edge labels or node paths).
    # If items are ints (node indices), coerce to letters before formatting.
    coerced_solutions = []
    ...
    all_solutions_html = [format_list(sols) for sols in coerced_solutions]
...
#export_exercise(partial(gs.find_all_hamiltonian_cycles_starting_at_one_node, start_node=start_node), "Find Hamiltonian Cycles", "a", "hamiltonian cycle", "FindHamiltonianCycles.xml")
export_exercise(partial(gs.find_all_hamiltonian_paths_starting_at_one_node, start_node=start_node, cycles=False), "Find Hamiltonian Paths", "a", "hamiltonian path", "FindHamiltonianPaths.xml")
...
def export_exercise_tf_cycle(graphs, start_node, filename):
    ...
    cycles_per_graph = [gs.find_all_hamiltonian_node_paths_starting_at_one_node(g, start_node, cycles=True) for g in graphs]
    ...
    variable_declarations += format_variable_declaration("variant_pos", id + 2, "randomIntegerBetween(0,sizeOfList([var=graphs]) - 1)")
    variable_declarations += format_variable_declaration("graph", id + 4, "getFromList([var=variant_pos], [var=graphs])")
    ...
    variable_declarations += format_variable_declaration("feedback_cycles", id + 10, "getFromList([var=variant_pos], [var=all_cycles_html])")
    ...
    answer_lists_code = "list(" + ", ".join("list(" + ", ".join(f"'{a}'" for a in al) + ")" for al in answer_lists) + ")"
    variable_declarations += format_variable_declaration("answer_options_all", id + 12, answer_lists_code)
    variable_declarations += format_variable_declaration("current_answer_options", id + 14, "getFromList([var=variant_pos], [var=answer_options_all])")

    # Provide answer_text_1..2 variables
    variable_declarations += format_variable_declaration("answer_text_1", id + 16, "getFromList(0, [var=current_answer_options])")
    variable_declarations += format_variable_declaration("answer_text_2", id + 18, "getFromList(1, [var=current_answer_options])")
    ...
    variable_declarations += format_variable_declaration("correct_answer_indices", id + 22, "getFromList([var=variant_pos], [var=correct_answer_indices_all])")
